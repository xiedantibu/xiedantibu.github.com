...
var result_label1='<img src="/assets/loader.gif" alt="loading" /> 获取中(专辑需时稍长,不要关闭本页)...';
...
{ 
	if(window.clipboardData){ 
		...
		clip.on('complete', function(client, args) {
			var btn = $(this);
			var oldhtml = btn.html();
			$(this).html('已复制到剪切板').attr('disabled','disabled');
...
window.getFlv = function() {
  ...
  $('#resultbox').show();
	$.getScript("http://www.flvxz.com/getFlv.php?url="+encodeUri(uri), function(){
		$('#fetchButton').attr('disabled',false);
		$('#fetchButton').val(button_label0);
		$('#apiurls').html('<a class="btn btn-mini" href="http://api.flvxz.com/url/'+encodeUri(uri)+'" target="_blank" title="xml格式的输出，目前免费">xml</a><a class="btn btn-mini" href="http://api.flvxz.com/url/'+encodeUri(uri)+'/xmlformat/ckxml" target="_blank" title="xml格式for ckplayer">ckxml</a>');
	});
...
window.flvout = function(d,resultid){
	...
	if(typeof resultid == "undefined"){
		...
		var tip = '结果稍后出现...'; // 不能包含loading，因为#result判断用了loading
		var ri = $('#'+resultid);
